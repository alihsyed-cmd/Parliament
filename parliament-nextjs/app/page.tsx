...
// app/page.tsx — Parliament. Client-side state machine that orchestrates the
// four screens and all data fetching through lib/api.
//
// Routing note: this is a single client view with in-memory navigation, which
// keeps the app feeling native (and trivial to wrap in Capacitor). If you want
// web deep-links / SSR later, split these into App Router routes:
//   app/page.tsx            → Entry
//   app/reps/page.tsx       → Lookup (postal in a query param or POST body)
//   app/j/[slug]/page.tsx   → Roster
//   app/r/[jur]/[slug]      → Detail
// The screen components and lib/* stay identical; only the routing wrapper changes.
...
export default function Page() {
  ...
  const openRep = React.useCallback(async (rep: Politician, level: Level, fromRoster = false) => {
    ...
    setRepDetail(null);
    setRoute("detail");
    // Fetch full representations if we have slugs (falls back silently to embedded data).
